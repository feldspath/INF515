...
void main() {
    vec3 direction = camera_direction(2 * vec2(-gl_FragCoord.x/width, -gl_FragCoord.y/height) + vec2(1.0f));
    ...
    if (light_distance > length(light_pos - sphere_intercept)) {
        ...
        specular_color = specular * pow(max(dot(light_reflection, direction), 0.0f), roughness) * vec3(1.0f);
    }
